...
impl SpectrumWaveform {
    pub fn new(bands: [f32; 8], color: Color) -> Self {
        Self {
            bands,
            color,
            bar_width: 3.0,
...
{
    fn size(&self) -> Size<Length> {
        let total_width = (self.bands.len() as f32) * (self.bar_width + self.bar_spacing) - self.bar_spacing;
        Size {
            width: Length::Fixed(total_width),
    ...
    fn layout(
        ...
        let size = limits
            .width(Length::Fixed(total_width))
            .height(Length::Fixed(self.total_height))
            .resolve(Length::Fixed(total_width), Length::Fixed(self.total_height), Size::ZERO);
...
pub fn spectrum_waveform(bands: [f32; 8], color: Color) -> SpectrumWaveform {
    SpectrumWaveform::new(bands, color)
}
