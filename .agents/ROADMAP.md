...
- **Insert delivery target** — landed with semantic Wayland input-method insertion, explicit clipboard/stdout fallback reporting, and delivery policy tests. Follow-up work should focus on insertion module boundaries and real daemon/compositor verification, not fake insert UI.
...
1. **Execute the insertion-boundaries refactor plans** — insert delivery has
   landed; the next value is making `delivery.rs`, `insertion/`, and
   `insertion/backends/wayland/` match the intended responsibilities before
   adding any more insertion behavior.
