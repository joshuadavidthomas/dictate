...
|---|---|---|---|---|---|---|---|---|---|---|
| Formatter × native ASR punctuation compatibility | correctness / direction | Sole blocker on the Parakeet default; fix is already characterized in the handback | Unblocks default flip → retires 30s truncation → unblocks partials spike | boundaries (model output is boundary data the formatter must translate per-model) | plan 004 handback (`Hello,, world,.` repro); `src/text.rs` formatter + inline insta snapshots | One plan: dedup/ignore ASR-attached punctuation adjacent to spoken commands, seeded with the handback's real Parakeet transcripts as characterization tests; include a headless `dictate transcribe <wav> [--raw\|--formatted]` subcommand (seams exist: `audio::load_wav_utterance` → transcription seam → formatter) so agents can iterate on the fix against real model output without a mic or human *(user seed: agentic feedback loop)* | Dedup rules have judgment calls (e.g. "period" after a model-emitted `.`); no surveyed tool documents keeping both layers *(research)* — "spoken commands off by default under punctuating models" is a live design option | Design review of the dedup rules; routine execution after | High | **Planned 2026-07-03** → `plans/formatter-punctuation-compat/` (001 transcribe CLI, 002 dedup fix, 003 spoken-command fixtures) |
| Daemon failure-contract hardening | correctness | Every future daemon feature stacks on this; three verified defects share one fix shape | Transient mic/download errors stop bricking the session; honest `deliver` contract; no wedged `Transcribing` | error-handling / effects | `src/daemon.rs:154,183,199` (worker `?` → terminal `mark_unavailable`, no exit transition in `src/dictation.rs:256`); `src/delivery.rs:28-43` (unreachable `Err`); `src/delivery.rs:46` (`println!` EPIPE panic skips state reset); `src/models.rs:162-181` (truncated download treated as success); `src/mic.rs:213` (silent ring-buffer drop); `src/daemon.rs:93-99` (accept-error hot spin) | One plan batch: classify fatal (model init) vs retryable (mic open, stream error, delivery); return to Idle on retryable; make `deliver` infallible or per-utterance; verify download length; count dropped samples; backoff on accept error | Retry must not spin on a genuinely absent mic; touches the state machine hardening 003–005 already reworked (good tests exist) | Routine execution once the fatal/retryable classification is reviewed | High | `roadmap-to-improve-plans` |
| Re-true README + PLAN.md; document `config.toml` | docs | Docs are actively wrong about the two features the thesis calls the top gaps | Configurability is invisible: no user-facing mention that `~/.config/dictate/config.toml` exists | N/A | `README.md:17,23` claims stdout-only/"next focus copy" though clipboard + settings landed (`src/delivery.rs`, `src/settings.rs:23-39`, `src/cli.rs`); `PLAN.md` structure tree omits `settings.rs`/`delivery.rs`/`audio.rs` | Single small plan (or direct edit): README current-state list, config example, `--delivery` override, restart-required note | None | Routine execution | High | `roadmap-to-improve-plans` |
| Ship the unpushed work | direction / housekeeping | ~17 commits ahead of origin plus 4 unbookmarked harness commits; all completed, reviewed-by-plan work | Backup, CI runs, and a clean base for the Now plans | N/A | `jj log`: `main` `e6ce38bb` vs `main@origin` `692432f6`; stack `471e771f`…`fc9c3b87` above `main` | Push `main`; bookmark and push/PR the fixture-harness stack | None | Human approval (publishing) | High | `user decision` |
...
|---|---|---|---|---|---|---|---|---|---|
| Re-run Parakeet default flip (plan 004) | direction | Eval already passed everything except the formatter STOP | Default becomes the accuracy/long-form winner; 30s note retired; recording cap raised | N/A | plan 004 eval table + handback | Formatter fix landed; maintainer accepts ~1GB resident RSS | Human approval (RSS trade-off), then routine re-run of the existing plan | High | `user decision` → re-execute existing plan |
...
| Long-form fixture clips *(user seed)* | tests | Corpus is 7 clips of 1.3–9.7s (~25s total) — no fixture exercises the 30s decode ceiling, the >35s eval criterion, or the 120s cap | Once Parakeet is default, a >35s fixture makes the corpus test a permanent regression gate against ever reintroducing a 30s-window default | verification | Measured 2026-07-03 from `tests/fixtures/`; `tests/fixtures/manifest.toml` already records per-fixture provenance + transforms | **Ordering trap: a >30s fixture breaks `just test-integration` under the whisper-base-en default (silent truncation → WER over threshold) — land with or after the Parakeet re-run.** Cheapest source: concatenate committed CMU ARCTIC clips (same speaker, provenance already in the manifest, transform recorded as the concat command) into ~35s and ~90–120s fixtures; optionally one natural 20–35s LibriSpeech test-clean clip (CC BY 4.0, new corpus dir + LICENSE) | Routine execution | High | fold into the Parakeet re-run (plan 004) or the verification batch, whichever lands second |
| Model duration capability + VAD chunking *(added 2026-07-05)* | correctness / direction | Parakeet flip raised the recording cap to 10 min globally, but config can still select Whisper models that silently truncate at 30s — worse than the old 120s cap, since more speech is silently lost | Two stages: (a) catalog entries declare a single-pass duration limit and the daemon caps/warns accordingly — cheap honesty fix; (b) VAD-segmented chunking (silero VAD → split at speech boundaries → per-segment offline decode → stitch) removes the limit entirely and flips Whisper's capability to "unlimited via chunking". sherpa-onnx docs recommend exactly this pattern for long files; `silero_vad.onnx` already ships in the model dir. Chunking also bounds decode memory for any model and builds segmentation infrastructure adjacent to the partials spike (006) | boundaries (model window limit is a catalog fact, not a formatter/daemon assumption) | `src/dictation.rs:12` cap now 600s; plan 004 eval table (Whisper 30s truncation); sherpa-onnx long-file VAD examples | Capability field: none — land immediately (can ride with the long-form fixture work). Chunking: own small-to-medium plan; boundary/stitching rules (pauses mid-sentence, whitespace/punctuation joins, formatter interaction) need design review; needs the >35s fixtures as its regression gate | Design review of segment-stitching rules; routine after | High (capability field) / Medium (chunking design) | capability field: fold into long-form fixture batch; chunking: `feature-planning-artifacts` |
...
| Agentic feedback loop (user seed) | Verifying dictation behavior today needs a human to dictate live and watch the overlay — agents (and CI) are locked out of exactly the flows that matter most | Standing policy: every feature plan names how an agent verifies the behavior headlessly (fixture WAV through a CLI seam, snapshot, socket assertion, capture-and-exit debug scenario). Concrete substrates, in leverage order: `dictate transcribe <wav>` CLI (rides with the formatter fix, Now #1); dual-use debug harness with `--scenario X --capture out.png --exit` (Next); daemon audio injection (`record start --from-file x.wav`) so the full socket→phase→transcribe→deliver pipeline is agent-drivable; socket ack protocol (Later) so agents can assert daemon state instead of scraping stderr | Fixture corpus + model-backed harness already agent-runnable (`just test`, `just test-integration`); the gaps are live-daemon flows and the overlay | Record as a sentence in AGENTS.md; enforce via plan templates |
| Model punctuation capability flag | "Does this model emit native punctuation?" will be asked again by the formatter, partials, and any future model | Catalog entries declare punctuation behavior (`native / restore / none`); formatter consumes it instead of assuming Whisper; also gates the Later `OnlinePunctuation` fallback *(research)*. *(2026-07-05)*: extend the same capability shape to declare a single-pass duration limit (Whisper ~30s vs Parakeet ~24 min) — see the duration/chunking row in Next | plan 004 handback lingering question; research borrow-list item 3 gives it a second concrete consumer; post-flip, config-selected Whisper models silently truncate under the 10-min cap | Decide inside the formatter-fix plan; promote to catalog field only if the fix needs it |
...
| `plans/product-direction/004` | Keep, re-run pending | BLOCKED handback is the seed for Now #1; eval table stays the model-selection reference | Re-run Step 4 + flip after formatter fix |
...
| `PLAN.md` | Revise | Structure tree and delivery/settings claims stale; behavior-keep inventory still valuable | Fold into docs pass (Now #3) |
...
1. **`roadmap-to-improve-plans` for the Now batch** — three plans: (a)
   formatter × native-punctuation compatibility with handback-seeded
   characterization tests **plus the headless `dictate transcribe <wav>`
   subcommand as its verification substrate**, (b) daemon failure-contract
   hardening, (c) docs re-true. (a) is the critical path; (b) and (c) are
   independent of it and of each other.
2. **Two maintainer decisions, before any Next planning**: accept ~1GB
   resident RSS for Parakeet v2 as default (plan 004 handback), and answer
   the insertion spike's four open questions (`spike-insertion-findings.md`
   §Open questions). Both are pure judgment calls no agent should make.
3. **Push the completed work** — `jj git push` for `main`, bookmark the
   fixture-harness stack and open its PR; prune dead bookmarks while there.
...
Routine executors can run everything in Now except the push (human) and the
dedup-rule design corner of the formatter plan (design review). Nothing in
